...
class Curves:
        ...
        #then try to create models from given params
            ...
            for params in model_params:
                try:
                    ...
                    self.model.append(v)
                    if (g,a,b) == (0,2,2):
                        self.model_name.append('modified classical')
                    elif (g,a,b) == (1,1,2):
                        self.model_name.append('N-body: alternative')
                    else:
    ...
    def fit_galaxy(self,galaxy='ngc2403',method='scipy',return_plot=True,return_results=False,return_tests=False,
                   return_plot_data=False,guess=None,guess_scale=False,figsize=(10,10)):
        ...
        if return_plot:
            ...
            if len(df['rad'])>40:
                widths = [5,5,5,5]
                alphas = [.5,.5,.5,.5]
                points_markersize, points_capsize, points_linewidth = 1.5,1,1
                residuals_size = 150
            else:
                widths = [5,5,5,5]
                alphas = [.5,.5,.5,.5]
                points_markersize, points_capsize, points_linewidth = 6,4,4
                residuals_size = 250
            """
            
            divisor = max(len(df['rad']),20)
            
            widths = [5,5,5,5]
            alphas = [.5,.5,.5,.5]
            points_markersize, points_capsize, points_linewidth = 240/divisor, 160/divisor, 100/divisor
            residuals_size = 10000/divisor
            
            plt.figure(figsize=figsize)
            print(galaxy.upper()+": fits [{} - {}]".format(self.data_folder[5:-1],self.method_name[method]))
            #plt.title(galaxy.upper(),fontsize=scale*3*figsize[0])
            #plt.title(galaxy.upper()+": fits [{} - {}]".format(self.data_folder[5:-1],self.method_name[method]),fontsize=1.5*figsize[0])
            
            plt.xlabel('Raio (kpc)',fontsize=scale*1.875*figsize[0])
            plt.ylabel('Velocidade (km s⁻¹)',fontsize=scale*1.875*figsize[0])
            
            plt.xticks(fontsize=scale*1.25*figsize[0])
            plt.yticks(fontsize=scale*1.25*figsize[0])

            #data used with errors
            plt.errorbar(df['rad'],df['vel'],yerr=df['vel_error'],fmt='o',markersize=points_markersize,capsize=points_capsize,linewidth=points_linewidth,color='black',alpha=0.9)
            
            #plot models
            lineshapes = ['-','-','-','-']
            for i in range(len(self.model)):
                plt.plot(df['rad'],yfits[i],lineshapes[i],linewidth=widths[i],markersize=widths[i],label=self.model_name[i],color=palette[i],alpha=alphas[i])

            #plt.xlim(0,df['rad'].iloc[-1]*1.25)
            plt.ylim(0,df['vel'].iloc[-1]*1.5)
            plt.legend(fontsize=leg_scale*1.5*figsize[0],loc='lower right') #loc='lower right' para NGC2903 e NGC6503
            plt.show()

            #plot residuals
            
            plt.figure(figsize=figsize)
            print(galaxy.upper()+": residuals and tests [{} - {}]".format(self.data_folder[5:-1],self.method_name[method]))
            #plt.title(galaxy.upper(),fontsize=scale*3*figsize[0])
            #plt.title(galaxy.upper()+": resíduos e testes [{} - {}]".format(self.data_folder[5:-1],self.method_name[method]),fontsize=1.5*figsize[0])
            
            plt.xlabel("Radius (kpc)",fontsize=scale*1.875*figsize[0])
            plt.ylabel("Residual (km s⁻¹)",fontsize=scale*1.875*figsize[0])
            ...
            markers = ['o','o','o','o']
            for i in range(len(self.model)):
                plt.scatter(df['rad'],residuals_tests[i],s=residuals_size,marker=markers[i],label=self.model_name[i],color=palette[i],alpha=0.4)
            
#             #try to plot results togheter
#             y_pos = np.linspace(0,-max_y,5*len(self.model)+1)
#             for i in range(len(self.model)):
#                 if self.model_name[i] == 'tradicional':
#                     try:
...
#                                  fontsize=4*figsize[1]/len(self.model),color=palette[i])
#                         plt.text(df['rad'].iloc[-1]*1.05,y_pos[5*i+1],'Modelo {}'.format(self.model_name[i]),
...
#             plt.title('Fits results',fontsize=2.5*figsize[0])
#             y_pos = np.linspace(0.9,0.1,4*len(self.model))
#             #print('RESULTADOS:\n')
#             for i in range(len(self.model)):
#                 #print('Modelo: {}'.format(self.model_name[i]))
#                 #print('Qui sem graus: {}'.format(chi_tests[i]))
#                 #print('Qui com graus: {}'.format(chi_degrees[i]))
#                 #print('\n')
#                 plt.text(0.1,y_pos[4*i],'Modelo {}'.format(self.model_name[i]),fontsize=5*figsize[1]/len(self.model),color=palette[i])
...
#             plt.show()
    ...
    def fit_all_galaxies(self,method='scipy',galaxies=None,
        ...
        for j in range(len(galaxies)):
            ...
            axes[y_ax,x_ax].set_title(galaxies[j].upper()+": fits [{} - {}]".format(
            ...
            axes[y_ax,x_ax].set_ylabel('Velocity (km s⁻¹)',fontsize=1.875*figsize[0]/x_len)
            ...
            for i in range(len(self.model)):
                try:
                    ...
                    axes[y_ax,x_ax].text(df['rad'].iloc[-1]*1.05,y_pos[5*i+2],
                             '(R0, P0): ({}, {})'.format(round(popts[i][0],2),round(popts[i][1],2)),
                             fontsize=4*figsize[1]/len(self.model)/y_len,color=palette[i])
                    axes[y_ax,x_ax].text(df['rad'].iloc[-1]*1.05,y_pos[5*i+1],'Modelo {}'.format(self.model_name[i]),
    ...
    def test_initialparams(self,method='scipy',galaxy='ngc2403',error='chi_square_with_degree',
        ...
        for i in range(len(self.model)):
            plt.figure(figsize=figsize)
            plt.title(galaxy.upper()+': estimation {} [{} - {}}]'.format(self.model_name[i],self.data_folder[5:-1],self.method_name[method]),fontsize=1*figsize[0])
